...
- **Compatibility:** Align plugin context logger usage with `@budarin/pluggable-serviceworker` types: use `context.logger.warn` (logger is required in `PluginContext`).

## [1.2.2] - 2026-03-02

### Changed

- **Compatibility:** Dev dependency `@budarin/pluggable-serviceworker` updated to ^1.17.0. Plugin remains compatible; no code changes (uses only `context.logger`).

## [1.2.1] - 2026-02-27

### Changed

- **Compatibility:** Verified with `@budarin/pluggable-serviceworker` 1.16.x. No code changes required; plugin does not use `fetch`/`fetchPassthrough`, only Cache API.
